...
| 3× RTX PRO 6000 Blackwell 96GB (vLLM BF16) | 28.3 | 73.4 | 3 | 0.66-0.83 | [@VestigeClub](https://github.com/VestigeClub) |
...
\* 3× RTX PRO 6000 Blackwell (vLLM BF16) row: three RTX PRO 6000 Blackwell Workstation Edition 96GB (400W, not Max-Q), vLLM 0.27.1, [huihui-ai/Huihui-Qwen3.8-27B-abliterated](https://huggingface.co/huihui-ai/Huihui-Qwen3.8-27B-abliterated) BF16 weights, BF16 KV, **524288 context** (official YaRN factor 2.0), DP=3 TP=1, Linux/CUDA, driver 595.84 / CUDA 13.2. Loaded VRAM ~90.6 GiB/GPU both arms (MTP head already in the BF16 checkpoint). Single stream, thinking off, only `--speculative-config` changed (`num_speculative_tokens` omitted vs 3). Method differs from stock `probe.py` on the MTP arm: unchanged `probe.py` at `70a699e` is accurate on spec-off (28.3 event/s = 28.3 tok/s, one token per SSE delta) but undercounts vLLM MTP because this engine coalesces accepted drafts into multi-token SSE deltas (probe reported 22.8 event/s while wall clock for the same 3×3 run was 92 s spec-off vs 39 s spec-on). Comparable number is `usage.completion_tokens` / decode-span on the same three prompts × three runs, warmup discarded: overall median 28.3 → 73.4 (+159%); per-prompt token medians python 86.9, prose/mmap 52.9, bash 73.4. Acceptance 0.66-0.83 (aggregate 0.74) from vLLM `spec_decode_num_accepted_tokens_total` / `spec_decode_num_draft_tokens_total` on the MTP-on process. Not a GGUF row — not comparable to the Q4 llama.cpp or NVFP4 vLLM footnotes on the other PRO 6000 rows.
