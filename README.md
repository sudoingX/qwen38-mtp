...
| RTX 3080 10GB (UD-IQ2_XXS, 8K, headless) | 44.8 | 65.7 | 2 | 0.47-0.94 | [@ntindle](https://github.com/ntindle) |
...
\* RTX 3080 10GB row: the smallest-VRAM single card in the table. unsloth UD-IQ2_XXS (8.39 GiB), **8K context**, q8_0 KV cache, llama.cpp b10435 (`9e40df63b`), Windows 11 / CUDA 12.4, Ryzen 9 7900X + 32 GB DDR5, full offload (`-ngl 99`, every layer), `--parallel 1`, thinking off, both arms otherwise identical. **The card is headless**: both monitors were moved off it onto the 7900X iGPU, which released 1.8 GB the desktop compositor was holding and is what makes full offload possible at all — at `ngl=62` this same box baselines 35.1. VRAM 8,969 MiB baseline / 9,660 MiB with spec (+691 MiB) of 10,240 MiB. Method: unchanged `probe.py` at `70a699e`, **medians of three complete passes** per rule from the 3090 sweep (baseline 45.4/44.8/44.7; n-max 2 64.4/65.7/68.5). Aggregate acceptance 0.74 (4,790/6,516 from server logs). n-max 3 measured 62.8 at 0.64 aggregate, n-max 4 fell to 55.6 at 0.55 — n-max 2 is the peak, matching rule 1. **On a 10 GB card the context ceiling is set by the MTP arm's extra 691 MiB, not by whether the baseline fits**: at 16K and 32K the identical flag inverts to -43% and -48% while acceptance *stays high* (0.66 and 0.75), and `nvidia-smi` still reports ~550 MiB free in every case. Full study, including the Windows-specific residency check that does detect it, in [sweeps/rtx-3090.md](sweeps/rtx-3090.md).
