...
| RTX PRO 6000 Blackwell 96GB (vLLM) | 63.3 | 96.1 | 2 | 0.59 | [@commdata2338](https://github.com/commdata2338) |
...
\* RTX PRO 6000 row: DIFFERENT ENGINE — vLLM 0.27.1, not llama.cpp: unsloth NVFP4 (compressed-tensors, not GGUF), 262K context, fp8 KV cache, thinking off, `--speculative-config '{"method":"mtp","num_speculative_tokens":2}'`. n-max 4 on the same box: 116.3 (+84%, acceptance 0.42) — same deeper-drafts shape as the sweeps below. Method: probe.py-equivalent streaming client (per-delta clocking, per-prompt medians, 5 mixed prompts, thinking off). Caveat for vLLM users: one server session intermittently dropped words from STREAMED output at quote boundaries with MTP on (non-streaming clean, token counts unaffected — invisible to throughput probes); tracked at [vllm-project/vllm#52469](https://github.com/vllm-project/vllm/issues/52469). Diff a streamed quoted sentence against its non-streamed twin before adopting.
