...
| RTX 3080 10GB (UD-IQ2_XXS, 16K, headless) | 44.6 | 63.8 | 2 | 0.47-0.92 | [@ntindle](https://github.com/ntindle) |
...
\* RTX 3080 10GB row: the smallest-VRAM single card in the merged table, and the only one where the weights are fully resident on under 12 GB. unsloth UD-IQ2_XXS (8.39 GiB), **16K context**, q8_0 KV cache, llama.cpp b10435 (`9e40df63b`), Windows 11 / CUDA 12.4, Ryzen 9 7900X + 32 GB DDR5, full offload (`-ngl 99`, every layer), `--parallel 1`, thinking off, both arms otherwise identical. VRAM 9,156 MiB baseline / 9,734 MiB with spec (+578 MiB) of 10,240 MiB. Method: unchanged `probe.py` at `70a699e`, **medians of three complete passes** per @dcrey7's note (baseline 45.1/44.6/44.6; n-max 2 63.8/65.8/63.7). Aggregate acceptance 0.75 (4,804/6,444 from server logs). n-max 3 measured 62.9 at 0.67 aggregate — n-max 2 is the peak, matching rule 1. 8K on the same box runs 44.8 → 65.7 (+47%) at 0.74. **The card must be headless and that is load-bearing**: with both monitors on the 7900X iGPU the desktop's footprint on the 3080 is 64 MiB and everything above holds; with them attached, dwm alone holds 1,846 MiB, full offload does not fit, and the same box baselines 33.0 at `ngl 62`. **The MTP arm's extra ~580 MiB, not the baseline's fit, sets the context ceiling**: 32K inverts to 45.4 → 24.7 (−46%) while acceptance *stays* at 0.73 and `nvidia-smi` still reports 446 MiB free. Full study — including the 115 MiB that moved that ceiling a full context step, and the Windows residency counter that detects it — in [sweeps/rtx-3090.md](sweeps/rtx-3090.md).
